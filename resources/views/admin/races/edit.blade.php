...
@section('content')
    <div class="row">
        <div class="card">
            <h5 class="card-title fw-semibold mb-4">{{ __('Edit Race') }}</h5>
            <div class="card-body">
                <form class="row col-sm-12 col-md-12 col-lg-12" action="{{ route('admin.races.update', $race->id) }}"
                    method="POST">
                    @csrf
                    @method('PUT')
                    <div class="col-md-6">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control" name="species_id">
                                ...
                                @foreach ($species as $id => $name)
                                    <option value="{{ $id }}" {{ $race->species_id == $id ? 'selected' : '' }}>
                                        {{ $name }}</option>
                                @endforeach
                            </select>
                            @error('description')
